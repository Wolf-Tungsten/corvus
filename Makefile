...
SCALA_FILE = $(shell find ./src/main/scala -name '*.scala') $(shell find ./XiangShan/src/main/scala -name '*.scala')
...
jar:
	mill -i corvus.assembly

comp:
	mill -i corvus.compile
	mill -i corvus.test.compile
...
clean:
	$(MAKE) -C ./XiangShan/difftest clean
	rm -rf $(BUILD_DIR)
...
init:
	...
	$(MAKE) -C ./XiangShan init GIT_FORCE_INIT=$(GIT_FORCE_INIT)
...
emu: emu-mk
	$(MAKE) -C ./XiangShan/difftest emu NUM_CORES=$(NUM_CORES) RTL_SUFFIX=$(RTL_SUFFIX)

# vcs simulation
simv: sim-verilog
	$(MAKE) -C ./XiangShan/difftest simv NUM_CORES=$(NUM_CORES) RTL_SUFFIX=$(RTL_SUFFIX)
simv-run:
	$(MAKE) -C ./XiangShan/difftest simv-run NUM_CORES=$(NUM_CORES) RTL_SUFFIX=$(RTL_SUFFIX)

# galaxsim simulation
xsim: sim-verilog
	$(MAKE) -C ./XiangShan/difftest xsim NUM_CORES=$(NUM_CORES) RTL_SUFFIX=$(RTL_SUFFIX)
xsim-run:
	$(MAKE) -C ./XiangShan/difftest xsim-run NUM_CORES=$(NUM_CORES) RTL_SUFFIX=$(RTL_SUFFIX)

# palladium simulation
pldm-build: sim-verilog
	$(MAKE) -C ./XiangShan/difftest pldm-build NUM_CORES=$(NUM_CORES) RTL_SUFFIX=$(RTL_SUFFIX)
pldm-run:
	$(MAKE) -C ./XiangShan/difftest pldm-run NUM_CORES=$(NUM_CORES) RTL_SUFFIX=$(RTL_SUFFIX)

pldm-debug:
	$(MAKE) -C ./XiangShan/difftest pldm-debug NUM_CORES=$(NUM_CORES) RTL_SUFFIX=$(RTL_SUFFIX)

bloop:
	mill --import ivy:com.lihaoyi::mill-contrib-bloop: mill.contrib.bloop.Bloop/install

.PHONY: FORCE verilog sim-verilog emu clean help init init-force bump bsp bloop $(REF_SO)
